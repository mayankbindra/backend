...
export default function ServiceForm(props) {
    ...
    useEffect(() => {
        ...
        if(Object.keys(props.editData).length != 0){
            ...
            setUrl(baseUrl+'/uploads/technologies/'+props.editData.image);
        }
      }, [props.token]);
    ...
    const submit = (e) => {
        e.preventDefault();  
        if(data.id){
            post(route("technologies_update", data.id));
        }else{
    ...
    return (
        ...
        <div className={`card ${ disabledForm == true ? 'disabledDiv':''}`}>
            <div className="card-header border-bottom">
                <h4 className="card-title">{data.id == "" ? 'Add' : 'Update'} Technology</h4>
            </div>
            <div className="card-body">
                <form className="validate-form mt-2 pt-50" method="post" encType="multipart/form-data" onSubmit={submit}>
                    <div className="row">
                        ...
                        <div className="col-12 col-sm-12 mb-1">
                            <label className="form-label" htmlFor ="content">Short Description</label>
